...
This application is available as ``app_slicekit_com_demo`` directory under ``sc_multi_uart`` component directory.

Demo Application Hardware
-------------------------

The echo demonstration application can be run on Slicekit core board. The following parts are required - 
...
Multi-UART component requires 8-bit ports for both UART transmit and UART receive ports. This component version runs on both L1 (one core) and also on L2 core based Slicekit Core boards, based on appropriate connector type selection as described. 
...
The TX handler operates by polling this buffer. When an entry is seen it pulls it from the buffer and perform action based on state of the handler. The TX handler will process that value on the correct UART identifier on the 8 bit port.
